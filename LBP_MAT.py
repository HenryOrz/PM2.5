...
# return type = list[][]
def calc_lbp(filename, mode="all"):
    compare_dict = {True: '1', False: '0'}
    im_mat = mat(misc.imread(filename, mode='L'))
    height, width = im_mat.shape
    lbp_mat_none = [[0 for j in range(width)] for i in range(height)]
    lbp_mat_rot = [[0 for j in range(width)] for i in range(height)]
    lbp_mat_uniform = [[0 for j in range(width)] for i in range(height)]
    ...
    for i in range(1, height - 1):
        for j in range(1, width - 1):
            ...
            val_mode_none = lbp_mode(lbp_val, 'none')
            val_mode_rot = lbp_mode(lbp_val, 'rot')
            val_mode_uniform = lbp_mode(lbp_val, 'uniform')

            lbp_mat_none[i][j] = val_mode_none
            lbp_mat_rot[i][j] = val_mode_rot
            lbp_mat_uniform[i][j] = val_mode_uniform

    return {"all": [lbp_mat_none, lbp_mat_rot, lbp_mat_uniform],
            "none": lbp_mat_none,
            "rot": lbp_mat_rot,
            "uniform": lbp_mat_uniform}[mode]


def lbp_mode(lbp_val, mode):
    if mode == 'none':
        return lbp_val
    if mode == 'lbprot':
        lbp_rot = {255: 255, 0: 0,
...
def lbp2json(lbp_mat, filename):
    json_mat = json.dumps(lbp_mat)
    with open(filename, 'w') as fs:
        fs.write(json_mat)


def json2lbp(filename):
    with open(filename, 'r') as fl:
        json_mat = fl.read()
    lbp_mat = json.loads(json_mat)
    return lbp_mat


def test(in_dir, out_dir):
    # data_log = open(out_dir + '\log\\' + 'data.log', 'ra')
    with open(out_dir + '\log\\' + 'lbp_opt.log', 'a') as opt_log, \
            open(out_dir + '\log\\' + 'lbp_data.log', 'a+') as data_log:

        # log start
        opt_log.write("\n#log start: " + time.strftime("%Y-%m-%d %H:%M:%S") + "\n")
        print("\n#log start: " + time.strftime("%Y-%m-%d %H:%M:%S") + "\n")

        # remove processed jpg
        ...
        for item in data_log:
            item = item[:-1]
            if item in jpg_list:
                jpg_list.remove(item)
                opt_log.write(item + " processed\n")
                print item + " processed"
        opt_log.write('\n'+"processing: "+str(jpg_list)+'\n\n')
        print '\n'+"processing: "+str(jpg_list)+'\n\n'
        ...
        for fn in jpg_list:
            if os.path.splitext(fn)[-1] == '.jpg':
                lbp_mat_none, lbp_mat_rot, lbp_mat_uniform = calc_lbp(in_dir + '\\' + fn, "all")
                lbp2json(lbp_mat_none, out_dir + '\json\\' + fn + '_lbp_mat_none.json')
                lbp2json(lbp_mat_rot, out_dir + '\json\\' + fn + '_lbp_mat_rot.json')
                lbp2json(lbp_mat_uniform, out_dir + '\json\\' + fn + '_lbp_mat_uniform.json')
                data_log.write(fn + '\n')
                opt_log.write(in_dir+'\\'+fn+"--->"+out_dir+'\\'+fn+".json"+'\n')
                print(in_dir+'\\'+fn+"--->"+out_dir+'\\'+fn+".json"+'\n')
        end = time.clock()
        opt_log.write('\n'+"Success! Totally "+str(size(jpg_list))+' pictures\n')
        opt_log.write('\n'+'Running time: %s Seconds' % (end - start)+'\n')
        opt_log.write('\n'+"#log_end: " + time.strftime("%Y-%m-%d %H:%M:%S")+'\n')

        print '\n'+"Success! Totally "+str(size(jpg_list))+' pictures \n\n'


test("C:\Users\Henry.Z\Desktop\PM2.5.TEST\in", "C:\Users\Henry.Z\Desktop\PM2.5.TEST\out")
