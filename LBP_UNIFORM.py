...
def lbp(filename):
    im_mat = array(misc.imread(filename, mode='L'))
    height, width = im_mat.shape
    chart = []
    chart.extend(lbp_cell(im_mat[:height/2, :width/2]))
    chart.extend(lbp_cell(im_mat[height/2:, :width/2]))
    chart.extend(lbp_cell(im_mat[:height/2, width/2:]))
    chart.extend(lbp_cell(im_mat[height/2:, width/2:]))
    return chart
...
def lbp_cell(cell):
    chart = [0 for x in range(59)]
    height, width = cell.shape
    for i in range(height/BLOCK_SIZE):
        for j in range(width/BLOCK_SIZE):
            block = cell[i * BLOCK_SIZE:(i + 1) * BLOCK_SIZE, j * BLOCK_SIZE:(j + 1) * BLOCK_SIZE]
            lbp_val = lbp_block(block)
            chart[lbp_val] += 1
    return chart


def lbp_block(block):
    num = ['0' for x in range(8)]
    compare_dict = {True: '1', False: '0'}
    center = block[2, 3]
    num[0] = compare_dict[block[1, 1] > center]
    num[1] = compare_dict[block[0, 2] > center]
    num[2] = compare_dict[block[1, 3] > center]
    num[3] = compare_dict[block[2, 4] > center]
    num[4] = compare_dict[block[3, 3] > center]
    num[5] = compare_dict[block[4, 2] > center]
    num[6] = compare_dict[block[3, 1] > center]
    num[7] = compare_dict[block[2, 0] > center]
    lbp_bin = ''.join(num)
    lbp_val = lbp_mode(int(lbp_bin, 2),'uniform')
    return lbp_val
...
def test(in_dir, out_dir):
    # if os.path.exists(out_dir+'\\lbp.txt'):
    #     os.remove(out_dir+'\\lbp.txt')
    for fn in os.listdir(in_dir):
        if os.path.splitext(fn)[-1] == '.jpg':
            chart = lbp(in_dir+'\\'+fn)
            save_lbp_chart(chart, out_dir+'\\lbp.txt')


test("D:\DataSet\Test\\1", "D:\DataOut\lbp")
shutil.copyfile("D:\DataOut\lbp\lbp.txt", "D:\DataOut\lbp\git\\1\lbp.txt")
test("D:\DataSet\Test\\2", "D:\DataOut\lbp")
shutil.copyfile("D:\DataOut\lbp\lbp.txt", "D:\DataOut\lbp\git\\2\lbp.txt")
test("D:\DataSet\Test\\3", "D:\DataOut\lbp")
shutil.copyfile("D:\DataOut\lbp\lbp.txt", "D:\DataOut\lbp\git\\3\lbp.txt")
test("D:\DataSet\Test\\4", "D:\DataOut\lbp")
shutil.copyfile("D:\DataOut\lbp\lbp.txt", "D:\DataOut\lbp\git\\4\lbp.txt")
test("D:\DataSet\Test\\5", "D:\DataOut\lbp")
shutil.copyfile("D:\DataOut\lbp\lbp.txt", "D:\DataOut\lbp\git\\5\lbp.txt")
